...
def process_pdf_logic(uploaded_file):
    ...
    try:
        ...
        if final_data:
            ...
            for r in final_data:
                ...
                if "PAYMENT" in r["Type"]:
                    ...
                    if is_debit_entry:
                        ...
                        is_ret_src_cat = "Return" in doc_details_for_pending.get(source_doc, {}).get("Type", "")
            ...
            reconc_summary_injected = []
            for date_key, group in itertools.groupby(final_data, key=lambda x: x["Date"]):
                group_list = list(group)
                recon_rows = []
                for r in group_list:
                    chk = str(r["Type"])
                    if "Reconciliation" in chk or "Advance Carried Forward" in chk or "Advance Utilized" in chk:
                        recon_rows.append(r)
                        
                reconc_summary_injected.extend(group_list)
                
                if recon_rows and date_key != "":
                    tot_adv = sum(float(r["Debit"]) for r in recon_rows if "Advance Utilized" in str(r["Type"]) and r["Debit"] != "")
                    tot_bill = sum(float(r["Credit"]) for r in recon_rows if "Bill Settled" in str(r["Type"]) and r["Credit"] != "")
                    tot_cf = sum(float(r["Credit"]) for r in recon_rows if "Advance Carried Forward" in str(r["Type"]) and r["Credit"] != "")
                    
                    if tot_adv > 0 or tot_bill > 0:
                        rem_str = f"Total Adv Used: {int(tot_adv):,} | Total Bills Paid: {int(tot_bill):,}"
                        if tot_cf > 0:
                            rem_str += f" | New Adv Created: {int(tot_cf):,}"
                        
                        reconc_summary_injected.append({
                            "Date": "", "Type": "-> BATCH SETTLEMENT SUMMARY", "Doc No": "", 
                            "Chq No": "", "Debit": "", "Credit": "", "Balance": "", 
                            "Remarks": rem_str
                        })
            final_data = reconc_summary_injected
            
            final_data.append({
                "Date": "", "Type": "CLOSING BAL", "Doc No": "", "Chq No": "", 
                "Debit": "", "Credit": "", "Balance": round(running_balance, 2), "Remarks": ""
            })
            ...
            if abs(diff) > 0.5:
                ...
                pending_invoices.append({
                    "Date": "-", "Doc No": "SYSTEM BAL", "Type": adj_type,
                    "Billed Amt": round(diff, 2), "Pending Amt": round(diff, 2)
                })

            header_info = {
                "Time": run_datetime, "Period": period, 
                "CustomerNo": customer_no, "CustomerName": customer_name
            }
            
            summary_info = [
                ("OPENING BAL", opening_balance), ("Sales Invoice", s_inv), ("PAYMENT", pay), 
                ("Reconciliation", reco), ("Credit Note(Others)", c_oth), 
                ("Credit Note(Brakage Expiry)", c_brk), ("Goods Return Invoice", g_ret),
                ("Debit Note", d_not), ("TCS Debit Note", tcs), ("TDS Credit Note", tds),
                ("TECHNICAL BOUNCED", tech_b), ("NON TECHNICAL BOUNCED", n_tech_b), 
                ("CLOSING BAL", running_balance)
            ]
            
            dash_info = {
                "Opening Bal": opening_balance, "Billed (Dr)": total_billed_dr, 
                "Paid / Adj (Cr)": total_paid_cr, "Closing Bal": running_balance, 
                "Total Pending": running_balance
            }
            return final_data, header_info, summary_info, dash_info, pending_invoices, None
            
    except Exception as e:
        return None, None, None, None, None, f"सिस्टीम कॅल्क्युलेशन एरर: {str(e)}\n{traceback.format_exc()}"

    return None, None, None, None, None, "डेटा सापडला नाही (Data not found)."
...
def get_pdf_download_fpdf(final_data, header_info, summary_info, dash_info, pending_invoices, manual_name="", cfa_name=""):
    ...
    for h in dash_headers: 
        pdf.cell(dash_w, 6, safe_str(h), border=1, align='C', fill=True)
    ...
    for i, v in enumerate(dash_vals):
        ...
        pdf.cell(dash_w, 8, safe_str(v), border=1, align='C')
    ...
    for i in range(len(headers)): 
        pdf.cell(col_widths[i], 6, safe_str(headers[i]), border=1, align='C', fill=True)
    pdf.ln()
    ...
    for r in final_data:
        ...
        if is_summary:
            ...
            pdf.set_xy(temp_x + col_widths[0], y_start)
            pdf.cell(col_widths[1], 6, safe_str(r['Type'])[:35], align='L')
            pdf.set_xy(temp_x + col_widths[0] + col_widths[1], y_start)
            ...
            cur_rect_x = temp_x
            for w in col_widths: 
                pdf.rect(cur_rect_x, y_start, w, row_height, style)
                cur_rect_x += w
            ...
            pdf.set_xy(temp_x, y_start)
            d_str = f"{int(float(r['Debit'])):,}" if r['Debit']!="" else ""
            pdf.cell(col_widths[4], 6, d_str, align='R')
            temp_x += col_widths[4]
            
            pdf.set_xy(temp_x, y_start)
            c_str = f"{int(float(r['Credit'])):,}" if r['Credit']!="" else ""
            pdf.cell(col_widths[5], 6, c_str, align='R')
            temp_x += col_widths[5]
            
            pdf.set_xy(temp_x, y_start)
            b_str = f"{int(float(r['Balance'])):,}" if r['Balance']!="" else ""
            pdf.cell(col_widths[6], 6, b_str, align='R')
            temp_x += col_widths[6]
    ...
    if pending_invoices:
        ...
        p_col_widths = [25, 40, 55, 35, 35]
        p_headers = ["Date", "Doc No", "Type", "Billed (INR)", "Pending (INR)"]
        pdf.set_font("Arial", 'B', 8)
        
        for i in range(len(p_headers)): 
            pdf.cell(p_col_widths[i], 6, safe_str(p_headers[i]), border=1, align='C', fill=True)
    ...
    note_txt = "Note: This is a computer generated statement. If you have any queries or discrepancies regarding this statement, please contact our CFA / Accounts team within 7 days of receipt. Thank you for your business!"
    pdf.multi_cell(190, 5, safe_str(note_txt), align='L')
...
def get_excel_download(final_data, header_info, summary_info, dash_info, pending_invoices, manual_name="", cfa_name=""):
    ...
    for r in final_data:
        excel_data.append({
            "Date": r["Date"], "Type": r["Type"], "Doc No": r["Doc No"], 
            "Chq/NEFT No": r["Chq No"], "Billed Amt (Dr)": r["Debit"], 
            "Paid Amt (Cr)": r["Credit"], "Balance": r["Balance"], "Remarks": r["Remarks"]
        })
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        pd.DataFrame([
            ["Opening Bal", "Billed (Dr)", "Paid / Adj (Cr)", "Closing Bal", "Total Pending"], 
            [dash_info['Opening Bal'], dash_info['Billed (Dr)'], dash_info['Paid / Adj (Cr)'], 
             dash_info['Closing Bal'], dash_info['Total Pending']]
        ]).to_excel(writer, sheet_name='Statement', index=False, header=False, startrow=8)
        
        pd.DataFrame(
            summary_info, columns=["Transaction Type", "Amount (INR)"]
        ).to_excel(writer, sheet_name='Statement', index=False, startrow=12)
...
if uploaded_files:
    for file in uploaded_files:
        with st.spinner(f"⏳ {file.name} प्रोसेस होत आहे... कृपया थोडा वेळ थांबा..."):
            try:
                data, h_info, s_info, dash_info, pending_inv, err = process_pdf_logic(file)
                if err: 
                    st.error(f"❌ त्रुटी (Error): {err}")
                else:
                    st.success(f"✅ {file.name} यशस्वीरीत्या प्रोसेस झाली!")
                    st.write("### 📊 Dashboard Summary")
                    
                    c1, c2, c3, c4, c5 = st.columns(5)
                    c1.metric("Opening Bal", format_inr(dash_info.get('Opening Bal', 0)))
                    c2.metric("Billed (Dr)", format_inr(dash_info.get('Billed (Dr)', 0)))
                    c3.metric("Paid (Cr)", format_inr(dash_info.get('Paid / Adj (Cr)', 0)))
                    c4.metric("Closing Bal", format_inr(dash_info.get('Closing Bal', 0)))
                    c5.metric("Total Pending", format_inr(dash_info.get('Total Pending', 0)), delta_color="inverse")
                    ...
                    if st.session_state.get(f"ready_{file.name}", False):
                        ...
                        with col1: 
                            st.download_button(
                                "📥 Excel Download", 
                                get_excel_download(data, h_info, s_info, dash_info, pending_inv, manual_name, cfa_name), 
                                f"{file.name}.xlsx", 
                                key=f"dl_xl_{file.name}"
                            )
                        with col2: 
                            st.download_button(
                                "📥 PDF Download", 
                                get_pdf_download_fpdf(data, h_info, s_info, dash_info, pending_inv, manual_name, cfa_name), 
                                f"{file.name}.pdf", 
                                key=f"dl_pdf_{file.name}"
                            )
            except Exception as e:
                st.error(f"❌ अरेरे! {file.name} मध्ये एक तांत्रिक त्रुटी (Error) आली: {str(e)}")
